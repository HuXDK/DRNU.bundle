...
def LiveTV():
	...
	js = HTTP.Request('http://www.dr.dk/nu/embed/live', values = {'height':'467', 'width':'830'} )
	Log.Debug(js)
	liveTVChannels = JSON.ObjectFromString(HTTP.Request(js).content.split("'liveStreams':")[1].split("};")[0])
...
def CreateVideoItem(items, title):
	...
	for item in items:
		...
		try:
			...
			Log('t')
#		except:
#			Log.Debug('Error getting mediafiles')
 
			
		### Skal der kigges på
		if ('isPremiere' in item and item['isPremiere'] is not None) | ('premiere' in item and item['premiere']):
			title += ' *premiere'
		Log.Debug(item['isPremiere'])
		Log.Debug(item['premiere'])
		## Name Hack for programs like TV-Avisen
